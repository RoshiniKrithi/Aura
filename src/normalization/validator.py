"""Layer Normalization Input and Parameter Validator for Aura LLM Architecture.

Enforces 3D shape constraints (B, T, d), feature dimension matching, non-empty batch checks,
epsilon positivity, and numerical instability detection (NaN / Inf).
"""
...
@dataclass
class LayerNormValidationResult:
    """Summary diagnostic report output from LayerNorm validation checks."""
...
class LayerNormValidator:
    """Validates input tensors and parameters for LayerNormalization.
    ...
    def __init__(self, d_model: int, eps: float = 1e-5) -> None:
        """Initializes validator with target model dimension and epsilon.
        ...
        """
        if d_model <= 0:
            raise LayerNormValidationError(f"d_model must be positive, got {d_model}")
        if eps <= 0:
            raise LayerNormValidationError(f"epsilon must be positive, got {eps}")
    ...
    def validate_input_embeddings(
        ...
        Returns:
            LayerNormValidationResult summary object.
        """
        result = LayerNormValidationResult(is_valid=True)
        ...
        if not result.is_valid:
            logger.error("LayerNorm input validation failed: %s", result.errors)
